...
export default function NewsList() {
    ...
    return (
        <div className="p-6 bg-gradient-to-br from-gray-50 to-white min-h-screen pb-2">
                ...
                >
                    {news.map((article, index) => (
                        <SwiperSlide key={index}>
                            <div className="group bg-white shadow-lg rounded-3xl overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100">
                                {article.imageUrl && (
                                ...
                                <div className="p-6">
                                    <h2 className="text-xl font-bold mb-3 leading-tight">
                                        <a
                                            ...
                                            className="text-gray-800 hover:text-blue-600 transition-colors duration-200 line-clamp-2"
                                        >
                                            {article.title}
                                        </a>
                                    </h2>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="flex items-center justify-between text-xs">
                                            <div className="flex items-center space-x-1 text-gray-500">
                                                ...
                                                <span className="truncate max-w-24">{article.creator || "Anonymous"}</span>
                                            </div>

                                            <div className="flex items-center space-x-1 text-gray-500">
                                                <span className="text-orange-500">📰</span>
                                                <span className="font-semibold text-blue-600 truncate max-w-20">{article.sourceName}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex flex-wrap gap-1 mt-3">
                                            {article.category.slice(0, 3).map((cat, idx) => (
                                                <span
                                                    key={idx}
                                                    className="inline-block px-2 py-1 bg-gradient-to-r from-blue-50 to-purple-50 text-blue-700 text-xs font-medium rounded-full border border-blue-100"
                                                >
                                                    {cat}
                                                </span>
                                            ))}
